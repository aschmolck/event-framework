(ns eventframework.app-test
  ...
  (:require
   ...
   [aleph.core :as foobar]
   )
  #_(:import [aleph.core AlephServer]))
...
(defn is-text-present? [text]
  (let [escaped-text (string/replace text "'" "&quot;")]
    (boolean (taxi/find-element {:xpath "//*[contains(.,'" escaped-text "')]"}))))
...
(deftest selenium-test
  ...
  (let [port 8887
        server-stop (aleph.http/start-http-server
                ...
                {:port port})
        ;; up (aleph.http/sync-http-request
        ;;      {:method :get
        ;;       :url (str "http://localhost" ":" port)
        ;;       :delimiters ["\r"]})
        ]
    ;; busy-loop till server is up
;    (taxi/implicit-wait 6000)
    (println "Server pretends to be up")
    (taxi/to (str "http://localhost" ":" port))
    (println "Taxied")
    (fact "has text"
      (is-text-present? "Proof-of-concept chat client") => true)
    (println "Ran tests")
    (server-stop)
